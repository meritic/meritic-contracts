...
contract Service is ERC3525, AccessControl {
    ...
    event CreditsConsumed(uint256 tokenId, uint256 creditValue, uint256 underlyingValue, uint256 royaltyValue);
    ...
    constructor(address revenueAddress_,
        ...
        _revenueAddress = revenueAddress_;
  		...
  		_underlying = IERC20(underlyingContract_);
  		...
  		_registry.registerContract(revenueAddress_, defaultSlot_, _contractType);
		...
		_setupRole(SERVICE_ADMIN_ROLE, revenueAddress_);	
    ...
    function setRoyaltyDistributor(address distributor_) external virtual {
        ...
        royaltyDistributor = RoyaltyDistributor(distributor_);
        // Centralized approval for any child contract using _underlying
        _underlying.approve(distributor_, type(uint256).max);
    }
    
    
    function valueContractAddress() public view returns (address){ return address(_underlying); }
    ...
    function defaultSlot() public view returns (uint256) { return _defaultSlot; }
    function _baseURI() internal view virtual override returns (string memory) { return ServiceMetadataDescriptor(address(metadataDescriptor)).baseURI(); }
	function contractURI() public view virtual override returns (string memory) { return ServiceMetadataDescriptor(address(metadataDescriptor)).constructContractURI(); }
    function tokenURI(uint256 tokenId_) public view virtual override returns (string memory) { ERC3525._requireMinted(tokenId_); return ServiceMetadataDescriptor(address(metadataDescriptor)).constructTokenURI(tokenId_); }
 	function _setBaseURI(string memory uri_) external virtual { ServiceMetadataDescriptor(address(metadataDescriptor)).setBaseURI(uri_); }
	
    function contractAdmin() public view returns (address){ return _revenueAddress; }
	
	function creditType() public view returns (string memory) {
	    string memory ctype;
	    ctype = (_contractType == Registry.CreditType.priority_ ? 'priority' : _contractType == Registry.CreditType.time_ ? 'time' : _contractType == Registry.CreditType.counts_ ? 'counts' : 'cash');
	    return ctype;
	}
    ...
    function _afterTokenTransfer(
  	...
  	function mint(address owner_, uint256 slot_, uint256 value_, string memory uuid_, string memory tokenDescription_, string memory tokenImage_, string memory property_) public virtual returns (uint256) {
        uint256 existingTokenId = 0;
        bool found = false;
        
        // ERC3525 from Solv implements balanceOf(address) and tokenOfOwnerByIndex
    ...
    function isContractToken(uint256 tokenId_) internal view returns (bool){ return (ERC3525._exists(tokenId_) && ERC3525.slotOf(tokenId_) == _defaultSlot); }
    function isInternalToken(uint256 tokenId_) internal view returns (bool) { return (networkTokenId[tokenId_] != 0); }
    function _isNetworkToken(uint256 tokenId_) internal view returns (bool) { return (networkTokenId[tokenId_] != 0 || _slotPool.isNetworkToken(tokenId_)); }
    function isExternalToken(uint256 tokenId_) internal view returns (bool) { return ( (networkTokenId[tokenId_] == 0 && _slotPool.contractOf(tokenId_) != address(this)) || (networkTokenId[tokenId_] != 0  && _slotPool.contractOf(networkTokenId[tokenId_]) != address(this)) ); }

    function transferFrom(address from_, address to_, uint256 tokenId_) public payable virtual override(ERC3525) {
        super.transferFrom( from_, to_, tokenId_);
    }
    
    function getTokenUUID(uint256 tokenId) view external returns(string memory){ return ServiceMetadataDescriptor(address(metadataDescriptor)).getTokenUUID(tokenId); }
    function getTokenDescription(uint256 tokenId) view external returns(string memory){ return ServiceMetadataDescriptor(address(metadataDescriptor)).getTokenDescription(tokenId); }
    function getTokenImage(uint256 tokenId) view external returns(string memory){ return ServiceMetadataDescriptor(address(metadataDescriptor)).getTokenImage(tokenId); }
	...
	function registerOnSlot(uint256 slotId_) public {
	    ...
	    _registry.registerContract(_revenueAddress, slotId_, _contractType);
	}
	
	function _exists(uint256 tokenId_) internal view virtual override returns (bool) { return ERC3525._exists(tokenId_) || _isNetworkToken(tokenId_); }
	function _requireMinted(uint256 tokenId_) internal view virtual override { require(_exists(tokenId_) || _isNetworkToken(tokenId_), "ERC3525: invalid token ID"); }
    
    function balanceOf(uint256 tokenId_) public view virtual override returns (uint256) {
        _requireMinted(tokenId_);
        if(isContractToken(tokenId_)){ return super.balanceOf(tokenId_); } else { uint256 netTokenId = networkTokenId[tokenId_]; return _slotPool.balanceOf(netTokenId); }
    }

    function ownerOf(uint256 tokenId_) public view virtual override(ERC3525) returns (address owner_) {
        _requireMinted(tokenId_);
        if(isContractToken(tokenId_)){ return super.ownerOf(tokenId_); } else { return _slotPool.ownerOf(networkTokenId[tokenId_]); }
    }
